...
Examples of correct code for the `{ "ignoreComponentAttribute": [{ name: "div", "attributes": ["foo"] }] }`:

```jsx
/*eslint i18next/no-literal-string: ["error", { "ignoreComponentAttribute": [{ name: "div", "attributes": ["foo"] }] }]*/
const element = <div foo="bar" />;
```
