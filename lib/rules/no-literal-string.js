...
module.exports = {
  ...
  create(context) {
    ...
    function isValidAttrName(tagName, name) {
      if (ignoreComponentAttribute.length) {
        const match = ignoreComponentAttribute.filter((item) => item.nodeName === tagName);
        const any = match.find((item) => item.nodeAttribute === name);
    ...
    const scriptVisitor = {
      ...
      JSXAttribute(node) {
        ...
        if (isValidAttrName(tagName, attrName)) {
          indicatorStack.push(true);
          return;
        }
