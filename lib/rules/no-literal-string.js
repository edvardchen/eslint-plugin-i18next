...
module.exports = {
  ...
  create(context) {
    ...
    function shouldIgnoreProperty(node) {
      const { key, value, computed } = node;
      // ignore properties in ignoreProperty option
      if (!computed && key && ignoredObjectProperties.includes(key.name)) {
        return true;
      }

      // don't care whether if this is computed or not
      // name if key is Identifier; value if key is Literal
      const propertyLiteral = key.name || key.value;

      // allow
      // var a = {["A_B"]: "hello world"};
      // var a = {[A_B]: "hello world"};
      // var a = {A_B: "hello world"};
      if (isUpperCase(propertyLiteral)) return true;
    ...
    const scriptVisitor = {
      ...
      'Property > Literal'(node) {
        ...
        if (shouldIgnoreProperty(parent)) {
          visited.add(node);
        }
      },

      'Property > ArrayExpression > Literal'(node) {
        const propertyNode = getNearestAncestor(node, 'Property');

        // allow
        // var a = {A_B: ["hello world"]};
        if (shouldIgnoreProperty(propertyNode)) {
          visited.add(node);
        }
      },
