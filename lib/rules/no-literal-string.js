...
function getAncestors(context, node) {
  if (context.sourceCode && context.sourceCode.getAncestors) {
    return context.sourceCode.getAncestors(node);
  }
  return context.getAncestors(node);
}

function withDottedPrefix(patterns = []) {
...
function isValidFunctionCall(context, options, { callee }) {
  ...
  const sourceText = getSourceCode(context).getText(callee);
  const { include = [], exclude = [] } = options.callees || {};

  return shouldSkip(
    {
      include: withDottedPrefix(include),
      exclude: withDottedPrefix(exclude),
    },
    sourceText
  );
}
...
module.exports = {
  ...
  create(context) {
    ...
    function report(node) {
      context.report({
        node,
        message: `${message}: ${getSourceCode(context).getText(node.parent)}`,
      });
    }
    ...
    const scriptVisitor = {
      ...
      JSXElement(node) {
        const fullComponentName = getSourceCode(context).getText(
          node.openingElement.name
        );
        const componentOptions = options['jsx-components'] || {};
        const { include = [], exclude = [] } = componentOptions;
        indicatorStack.push(
          shouldSkip(
            {
              include: withDottedPrefix(include),
