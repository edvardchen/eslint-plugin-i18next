        ...
        fix: function(fixer) {
          if (
            ...
            const replacement = node.value
              ...
              .trim();
            console.log(
              `fixing L${node.loc.start.line} "${node.value}" to {i18n.t('${replacement}')}`
            );
            return fixer.replaceText(node, `{i18n.t('${replacement}')}`);
          }
          if (node.type === 'Literal') {
            const replacement = `i18n.t(${node.raw})`;
            console.log(
          ...
          if (node.type === 'TemplateLiteral') {
            const formatStr = node.quasis
              ...
              .join('');
            const replacement = `i18n.t('${formatStr}', {${node.expressions
          ...
          if (node.value === undefined) {
            throw new Error('Unknown node type', node.value);
          }
          console.log('fixer', node, fixer);
