...
function updateDict(str, loc) {
  console.log(`${loc.fileName.slice(51)}:${loc.start.line}: "${str}"`);
        ...
        fix: function(fixer) {
          ...
          const forwardRefs = ast.body.filter(
            n =>
              n.declarations?.[0]?.init?.callee?.property?.name ===
                'forwardRef' &&
              n.declarations[0]?.init?.arguments[0]?.id?.name === 'InputIp'
          );
          ...
          if (exportedNamedDeclarations.length === 1) {
            ...
            functionComponentBody =
              forwardRefs[0].declarations[0].init.arguments[0].body.body;
          } else {
            console.warn(
